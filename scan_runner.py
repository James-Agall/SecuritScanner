"""
Shared scan pipeline: crawl + every scanner + persistence + report generation.

This is the single implementation of "run a full scan" used by both the CLI
(main.py, under `if __name__ == "__main__":`) and the FastAPI background task
(api/routers/scans.py) - so there is exactly one place that wires the 14
scanners together, and the CLI and the web API can never drift apart.
"""
